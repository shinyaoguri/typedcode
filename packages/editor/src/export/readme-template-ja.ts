...
このアーカイブには、TypedCodeからエクスポートされたタイピング証明データが含まれています。TypedCodeは、すべての編集操作を改ざん耐性のあるハッシュチェーンに記録するコードエディタです（署名チェックポイントがある場合はサーバ署名による時刻アンカリングも付く）。正確な保証範囲は後述の「証明できること / できないこと」を参照。
...
### 証明できること
- 改ざん耐性: エクスポート後に記録イベントを 1 ビットでも変更すると SHA-256 ハッシュチェーン / PoSW が壊れる
- エクスポートされた最終コードが記録された編集列と一致する（content replay）
- 時刻アンカリング（署名チェックポイントがある場合のみ）: チェーンがサーバ署名時刻に存在していたこと

### 証明できないこと
- タイピングした人物の身元（fingerprint は環境の識別であり個人ではない）
- 人間（スクリプトではない）がイベントを生成したこと（合成/注入イベントも同じく valid になる。挙動分析は advisory のみ）
- コードがオリジナルである（他から転記・打ち写しされていない）こと（リアルタイム転写は対象外）
- 外部からの支援が使用されていないこと

> 注: 「ピュアタイピング」と Turnstile 認証は読み手の判断材料であって暗号的保証ではない（認証は proof 検証時に再検証されない）。暗号的に保証されるのは改ざん耐性と、署名チェックポイントがある場合の存在時刻のみ。
