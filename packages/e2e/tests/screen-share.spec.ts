...
test('画面共有を有効化すると screenShareStart とスクショがチェーンに記録される', async ({ page }) => {
  ...
  expect(
    events.some((e) => e.type === 'screenShareStart'),
    'screenShareStart recorded'
  ).toBe(true);
  expect(events.filter((e) => e.type === 'screenshotCapture').length, 'screenshotCapture recorded').toBeGreaterThan(0);

  // export ZIP に screenshots/ が含まれる (実画像が同梱される)。
  const entries = await listZipEntries(zipPath);
  expect(
    entries.some((n) => /screenshots?\//i.test(n)),
    'zip should contain screenshots/'
  ).toBe(true);
