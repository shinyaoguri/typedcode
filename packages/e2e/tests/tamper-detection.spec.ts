 ...
 *
 * 検証は `--mode fast`: ここで試す改ざんはハッシュチェーン不整合として検出され、
 * PoSW 再計算は不要 (full の positive control は happy-path が担う)。
 */
test('改ざんした proof を CLI が拒否する (無改ざんは pass)', async ({ page }) => {
  ...
  const clean = runVerifyCli(cleanPath, ['--mode', 'fast']);
  ...
  const tampered = runVerifyCli(tamperedPath, ['--mode', 'fast']);
