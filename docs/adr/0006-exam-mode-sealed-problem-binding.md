...
- **Status**: Accepted
- **Date**: 2026-06-05 (Proposed) / 2026-06-07 (Accepted)
- **Deciders**: (PR 上の合意者 / レビュアー)
- **PR / Commit**: 設計 = 本 ADR。実装 = PR1 shared 暗号コア (`PROOF_FORMAT_VERSION` 1.0.0→1.1.0 と同時) / PR2 editor (解錠・genesis 束縛・問題表示・export best-effort) / PR3 verify-cli grader / PR4 verify(web) grader UI。

> 本 ADR は当初「設計合意の記録 (実装は後続 PR)」として Proposed で merge した。proof フォーマットとハッシュ根に焼き込まれる**不可逆な決定** (= 後から録り直せない) のため、実装前に確定させる方針だった。**2026-06-07 に PR1–4 で実装が landed したため Accepted 化**。クロスカット仕様は [docs/system-spec.md](../system-spec.md) の「試験モード」節に反映済み。
...
**実装済み (PR1–4, 2026-06-07):**
- shared 暗号コア: `examAuthorityKeys` レジストリ・封印パッケージの build/verify/decrypt (`@noble/hashes` の Argon2id + WebCrypto AES-256-GCM)・`computeExamChainRoot`・`verifyExamBinding`・`proof.exam` 型・`examOpened` イベント。`PROOF_FORMAT_VERSION` 1.0.0→1.1.0 (検証器は `proof.exam` の有無で root 式を分岐)。
  - **具体化した決定**: packageHash と署名は同一の canonical core (`manifest − {signature, publicKeyJwk}`) を対象に計算する (任意同梱 publicKeyJwk・署名値に packageHash が依存しないため)。
  - **Argon2id パラメータ確定**: `memKiB=65536 (64 MiB) / iterations=3 / parallelism=1` (manifest に保持し出題者が調整可)。
- editor: ブロック型開始ゲート (`.tcexam` 取込 + 監督コード → 署名検証 → 復号)・`initializeExam` で genesis 束縛・`#0 humanAttestation` (best-effort)・`#1 examOpened`・ProblemPanel 表示。**export 前認証も exam では best-effort 化** (Workers 不達でも提出 ZIP を出す = サーバを critical path に置かない)。
- grader: verify-cli (`--exam-package` / `--submitted-at`、任意) と verify(web) (試験束縛カード + 問題パッケージ読込) の双方に `verifyExamBinding` を配線。

**セキュリティ硬化 (post-acceptance, 2026-06-07 — `verifyExamPackageSignature`):**
実装レビューで見つかった2つの信頼モデルの穴を塞いだ。gate (editor) と grader (verify-cli/web) は同じ `verifyExamPackageSignature` を通るので、両方に効く。
- **信頼アンカーは常に registry**: 以前は manifest 同梱の `publicKeyJwk` があれば keyId が registry に無くても**埋め込み鍵で署名検証していた**(= 攻撃者が自分の鍵を同梱すれば自己署名の偽問題が trust される自己証明の穴)。修正後は **registry 未登録 keyId を untrusted で reject**。同梱鍵は long-term verifiability 用の控え扱いとし、registry にある場合のみ JWK 一致を必須にする (すり替え検出)。署名検証は常に registry 公開鍵で行う。
- **鍵の有効期間 / 失効を強制**: 以前は `status`/`validFrom`/`validUntil`/`revokedAt` を**未チェック**だった (revoke しても署名が通る)。修正後は package の `releaseTime` を anchor に、`signedCheckpoints` のキー有効性判定と同型のルールを適用 (validFrom 前・validUntil 後を reject、`revokedAt` 以降の release を reject、`revokedAt` より前は trust + warning、`status='revoked'` で `revokedAt` 無しは安全側で reject)。
- **残存限界 (正直な明記)**: anchor の `releaseTime` は**出題者署名の対象内 = 出題者の自己申告**であり、checkpoint の `serverTimestamp` のような独立した信頼時刻ではない。鍵が漏洩した攻撃者は `releaseTime` を遡らせて失効/期限切れ判定を回避し得る。完全な失効には proof 側の署名 cp の `serverTimestamp` を anchor にする必要がある (将来課題)。本硬化は「正直に申告された期限切れ/失効鍵を弾く」defense-in-depth に留まる。
...
- 出題者ツール (`generate-exam-authority-key.mjs` / `make-exam-package.mjs`) の正式運用フロー整備 (本番 `examAuthorityKeys/registry.ts` への鍵登録、配布手順)。
